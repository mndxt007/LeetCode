...
Continuing this process, the queue becomes [1] at 6 seconds.
The kth person has bought all their tickets, so return 6.
...
{
    int secondsNeeded = 0;
    Queue<(int index, int remainingTickets)> queue = new(
        tickets.Select((index, item) => (item, index))
    );
    while(queue.Count > 0)
    {
        var personObj = queue.Dequeue();
        secondsNeeded++;
        personObj.remainingTickets--;
        if(personObj.remainingTickets == 0)
        {
            if(personObj.index==k)
                break;
            continue;
        }
        queue.Enqueue(personObj);
    }
...
List<(int[] tickets, int k)> testcases = [
    ([2,3,2], 2),
    ([5,1,1,1], 0)
];

foreach (var (tickets, k) in testcases)
{
    Console.WriteLine($"Testcase: tickets-[{String.Join(',', tickets)}] k-{k}");
    Console.WriteLine($"TimeRequiredToBuy - {TimeRequiredToBuy(tickets, k)}");
}
