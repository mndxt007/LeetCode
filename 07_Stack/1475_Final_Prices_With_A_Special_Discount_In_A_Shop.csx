...
public int[] FinalPrices(int[] prices)
{
    ...
    {
        while(mono.Count > 0 && prices[mono.Peek()] >= prices[i])
        {
            var discountIndex=mono.Pop();
            prices[discountIndex]-=prices[i];
        }
...
foreach (var prices in testcases)
{
    Console.WriteLine($"Testcase: prices-[{String.Join(',', prices)}]");
    Console.WriteLine($"FinalPrices - [{String.Join(',', FinalPrices(prices))}]");
}
